...
public class JSONUtils {
	...
	public static int lengthLeftWhiteSpaceAndCommaColon(String string) {
		...
		if (first == ',' || first == ':') {
			return lenBeforePotentialComma + lengthLeftWhiteSpaceAndCommaColon(noLeftWhitespace.substring(1)) + 1;// add
																													// 1
																													// for
																													// char
		} else {
	...
	public static boolean nextNonWhiteCharIsClosing(String string) throws JSONException {
		return (nextNonWhiteCharIs('"', string) || nextNonWhiteCharIs('}', string) || nextNonWhiteCharIs(']', string));
	}
	...
	public static TreeMap<String, String> castJSONSingleValsToStrings(TreeMap<String, JSONSingleVal> mapOfJSONSingleVals) {
		TreeMap<String, String> otherValStrings = new TreeMap<String, String>();
		for (Entry<String, JSONSingleVal> entry : mapOfJSONSingleVals.entrySet()) {
			otherValStrings.put(entry.getKey(), entry.getValue().getVal());
		}
		return otherValStrings;
	}

	public static ArrayList<TreeMap<String, String>> castJSONSingleValsToStrings(
			ArrayList<TreeMap<String, JSONSingleVal>> treeMaps) {
		
		ArrayList<TreeMap<String, String>> justStrings = new ArrayList<TreeMap<String, String>>();
		for (TreeMap<String, JSONSingleVal> entry : treeMaps) {
			justStrings.add(JSONUtils.castJSONSingleValsToStrings(entry));
		}
		return justStrings;
	}
